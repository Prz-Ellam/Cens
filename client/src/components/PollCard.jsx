 ...
 * @returns {JSX.Element} Componente de tarjeta de encuesta.
 */
function PollCard({ poll, onUpdate }) {
  const {
    id,
    question,
    description,
  ...
  const submitCreateVote = async (pollId, optionId) => {
    try {
      await axios.post(`/polls/${pollId}/votes`, { optionId });
  ...
  const submitUpdateVote = async (voteId, optionId) => {
    try {
      await axios.put(`/votes/${voteId}`, { optionId });
  ...
  const createReaction = async (pollId, isLike) => {
    try {
      await axios.post(`/polls/${pollId}/reactions`, { isLike });
      onUpdate(id);
    } catch (error) {
      const errorText = axios.isAxiosError(error)
        ? error.response.data.message
  ...
  const updateReaction = async (reactionId, isLike) => {
    try {
      await axios.put(`/reactions/${reactionId}`, { isLike });
  ...
  return (
    <section className="bg-accent text-gray-300 p-4 rounded-lg shadow-lg">
        ...
        <div className="flex gap-2 py-2" key={option.id}>
          ...
          <div className="grow relative w-full h-8 outline outline-2 outline-gray-600 rounded-lg">
            ...
            >
              ...
              <p className="text-gray-300">{option.percentage.toFixed(0)}%</p>
            </label>
          </div>
        </div>
      ))}
      ...
      <footer className="flex gap-5 mt-3">
        <button
          className={classNames(
            'flex content-center gap-1 cursor-pointer hover:text-green-400 transition duration-150 ease-out hover:ease-in',
            {
        ...
        <button
          className={classNames(
            'flex content-center gap-1 cursor-pointer hover:text-red-400 transition duration-150 ease-out hover:ease-in',
            {
